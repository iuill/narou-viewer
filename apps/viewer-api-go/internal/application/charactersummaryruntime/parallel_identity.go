...
func (r *Runtime) GenerateParallelIdentity(ctx context.Context, config *store.ResolvedAIGenerationConfig, novelID string, upToEpisodeIndex string, seed []characters.GeneratedCharacter, batches []characterSummaryBatch, progressSink func(appcharactersummary.BatchProgress), pendingUnresolved []characters.GeneratedUnresolvedMention) ([]characters.GeneratedCharacter, characterSummaryGenerationState, []ai.UsageRequest, error) {
	startedAt := time.Now()
	generated, state, requests, err := r.generateOpenRouterCharacterSummaryParallelIdentity(ctx, config, novelID, upToEpisodeIndex, seed, batches, progressSink, pendingUnresolved)
	status := "ok"
	if err != nil {
		status = "error"
	}
...
func (r *Runtime) GenerateDiscoveryParallelCorrection(ctx context.Context, config *store.ResolvedAIGenerationConfig, novelID string, upToEpisodeIndex string, seed []characters.GeneratedCharacter, batches []characterSummaryBatch, progressSink func(appcharactersummary.BatchProgress), pendingUnresolved []characters.GeneratedUnresolvedMention) ([]characters.GeneratedCharacter, characterSummaryGenerationState, []ai.UsageRequest, error) {
	startedAt := time.Now()
	generated, state, requests, err := r.generateOpenRouterCharacterSummaryDiscoveryParallelCorrection(ctx, config, novelID, upToEpisodeIndex, seed, batches, progressSink, pendingUnresolved)
	status := "ok"
	if err != nil {
		status = "error"
	}
...
func (r *Runtime) generateOpenRouterCharacterSummaryParallelIdentity(ctx context.Context, config *store.ResolvedAIGenerationConfig, novelID string, upToEpisodeIndex string, seed []characters.GeneratedCharacter, batches []characterSummaryBatch, progressSink func(appcharactersummary.BatchProgress), initialUnresolved []characters.GeneratedUnresolvedMention) ([]characters.GeneratedCharacter, characterSummaryGenerationState, []ai.UsageRequest, error) {
	if config == nil {
		return nil, characterSummaryGenerationState{}, nil, errors.New("AI generation profile was not found.")
	}
	...
	runtimeBatches, err := r.parallelIdentityRuntimeBatches(ctx, config, novelID, upToEpisodeIndex, batches, initialUnresolved)
	if err != nil {
		return nil, characterSummaryStateFromAllocator(initialUnresolved, allocator), nil, err
	}
	extracted, usageRequests, unresolved, err := r.extractParallelIdentityCandidates(ctx, config, novelID, upToEpisodeIndex, runtimeBatches, progressSink, initialUnresolved)
	if err != nil {
		return nil, characterSummaryStateFromAllocator(initialUnresolved, allocator), usageRequests, err
	}
	...
	if err != nil {
		return nil, characterSummaryStateFromAllocator(unresolved, allocator), usageRequests, err
	}
	...
	return generated, characterSummaryStateFromAllocator(unresolved, allocator), usageRequests, nil
}

func (r *Runtime) generateOpenRouterCharacterSummaryDiscoveryParallelCorrection(ctx context.Context, config *store.ResolvedAIGenerationConfig, novelID string, upToEpisodeIndex string, seed []characters.GeneratedCharacter, batches []characterSummaryBatch, progressSink func(appcharactersummary.BatchProgress), initialUnresolved []characters.GeneratedUnresolvedMention) ([]characters.GeneratedCharacter, characterSummaryGenerationState, []ai.UsageRequest, error) {
	if config == nil {
		return nil, characterSummaryGenerationState{}, nil, errors.New("AI generation profile was not found.")
	}
	allocator, err := characters.LoadGeneratedCharacterIDAllocator(r.stateDir, novelID, seed)
	...
	discovered, discoveryUsage, err := r.discoverParallelIdentityNames(ctx, characterSummaryNameDiscoveryConfig(config), novelID, upToEpisodeIndex, runtimeBatches)
	if err != nil {
		return nil, characterSummaryStateFromAllocator(initialUnresolved, allocator), nil, err
	}
	...
	extracted, usageRequests, unresolved, err := r.extractParallelIdentityCandidatesWithKnown(ctx, config, novelID, upToEpisodeIndex, knownCharacters, runtimeBatches, progressSink, initialUnresolved)
...
func (r *Runtime) parallelIdentityRuntimeBatches(ctx context.Context, config *store.ResolvedAIGenerationConfig, novelID string, upToEpisodeIndex string, batches []characterSummaryBatch, pendingUnresolved []characters.GeneratedUnresolvedMention) ([]characterSummaryBatch, error) {
	runtimeBatches := []characterSummaryBatch{}
	for _, batch := range batches {
		remaining := append([]characterSummaryChunk{}, batch.Chunks...)
		for len(remaining) > 0 {
			runtimeBatch, nextRemaining, err := r.nextRuntimeBatch(ctx, config, novelID, upToEpisodeIndex, nil, batch, remaining, pendingUnresolved)
			if err != nil {
				return nil, err
			}
...
type parallelIdentityExtractionResult struct {
	...
	Unresolved   []characterSummaryUnresolvedMention
...
func (r *Runtime) extractParallelIdentityCandidates(ctx context.Context, config *store.ResolvedAIGenerationConfig, novelID string, upToEpisodeIndex string, batches []characterSummaryBatch, progressSink func(appcharactersummary.BatchProgress), initialUnresolved []characters.GeneratedUnresolvedMention) ([]parallelIdentityCandidate, []ai.UsageRequest, []characters.GeneratedUnresolvedMention, error) {
	return r.extractParallelIdentityCandidatesWithKnown(ctx, config, novelID, upToEpisodeIndex, nil, batches, progressSink, initialUnresolved)
}
...
func (r *Runtime) extractParallelIdentityCandidatesWithKnown(ctx context.Context, config *store.ResolvedAIGenerationConfig, novelID string, upToEpisodeIndex string, knownCharacters []characters.GeneratedCharacter, batches []characterSummaryBatch, progressSink func(appcharactersummary.BatchProgress), initialUnresolved []characters.GeneratedUnresolvedMention) ([]parallelIdentityCandidate, []ai.UsageRequest, []characters.GeneratedUnresolvedMention, error) {
	...
	runErr := runParallelIdentityLLMJobs(ctx, len(batches), func(requestCtx context.Context, index int) error {
		...
		if progressSink != nil {
			sinkMu.Lock()
			progressSink(appcharactersummary.BatchProgress{Phase: "start", Batch: batch})
			sinkMu.Unlock()
		}
		result, err := r.generateOpenRouterBatch(requestCtx, config, novelID, upToEpisodeIndex, knownCharacters, batch, initialUnresolved)
	...
	if runErr != nil {
		return candidates, usageRequests, unresolved, runErr
	}
	for _, result := range results {
		if result.Err != nil {
			return candidates, usageRequests, unresolved, result.Err
		}
	...
	return candidates, usageRequests, unresolved, nil
}

func parallelIdentityCandidatesFromDelta(novelID string, requestIndex int, batch characterSummaryBatch, delta characterSummaryDelta) []parallelIdentityCandidate {
	return parallelIdentityCandidatesFromDeltaWithKnown(novelID, requestIndex, batch, delta, nil)
}

func parallelIdentityCandidatesFromDeltaWithKnown(novelID string, requestIndex int, batch characterSummaryBatch, delta characterSummaryDelta, knownCharacters []characters.GeneratedCharacter) []parallelIdentityCandidate {
	...
	generated, _ := applyCharacterSummaryDelta(localNovelID, knownCharacters, delta, allocator)
...
func (r *Runtime) resolveParallelIdentityClustersOneShot(ctx context.Context, config *store.ResolvedAIGenerationConfig, novelID string, upToEpisodeIndex string, candidates []parallelIdentityCandidate) ([]parallelIdentityCluster, ai.UsageRequest, error) {
	...
	if maxTokens < characterSummaryMinimumCompletionTokens {
		return nil, ai.UsageRequest{}, fmt.Errorf("OpenRouter request has only %d output tokens available for character identity resolution; at least %d are required.", maxTokens, characterSummaryMinimumCompletionTokens)
	}
	temperature := 0.1
	...
	return completeParallelIdentitySingletons(decoded.Clusters, candidates), usage, nil
}
...
func (r *Runtime) discoverParallelIdentityNames(ctx context.Context, config *store.ResolvedAIGenerationConfig, novelID string, upToEpisodeIndex string, batches []characterSummaryBatch) ([]characters.GeneratedCharacter, []ai.UsageRequest, error) {
	if len(batches) == 0 {
		return nil, nil, nil
	}
	...
	for _, result := range results {
		if result.Err != nil {
			return nil, usageRequests, result.Err
		}
...
func (r *Runtime) discoverParallelIdentityNamesForBatch(ctx context.Context, config *store.ResolvedAIGenerationConfig, novelID string, upToEpisodeIndex string, requestIndex int, batch characterSummaryBatch) ([]parallelIdentityDiscoveredName, ai.UsageRequest, error) {
	startedAt := time.Now()
	systemPrompt := strings.Join([]string{
	...
	payload := map[string]any{
		"novelId":          novelID,
		"upToEpisodeIndex": upToEpisodeIndex,
		"batch":            characterSummaryBatchPromptPayload(batch),
...
func discoveredNamesToGeneratedCharacters(values []parallelIdentityDiscoveredName, fallbackEpisodeIndex string) []characters.GeneratedCharacter {
	...
	return mergeGeneratedCharacters(nil, generated)
}
...
func characterSummaryConfigWithModel(config *store.ResolvedAIGenerationConfig, modelID string) *store.ResolvedAIGenerationConfig {
	if config == nil {
		return nil
	}
	copied := *config
...
func characterSummaryNameDiscoveryConfig(config *store.ResolvedAIGenerationConfig) *store.ResolvedAIGenerationConfig {
	if config == nil {
		return nil
	}
	return characterSummaryConfigWithModel(config, config.CharacterSummaryNameDiscoveryModelID)
}

func characterSummaryBatchPromptPayload(batch characterSummaryBatch) map[string]any {
