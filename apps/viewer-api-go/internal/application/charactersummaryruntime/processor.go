...
type Workflow interface {
	GenerateAndSave(ctx context.Context, novelID string, upToEpisodeIndex string, resolvedOverride *store.ResolvedAIGenerationConfig, strategy string, progressSink func(appcharactersummary.BatchProgress)) error
}

type JobStore interface {
	Save(novelID string, job characters.Job) error
}
...
func (s filesystemJobStore) Save(novelID string, job characters.Job) error {
	return characters.SaveJob(s.stateDir, novelID, job)
}
...
func (p *Processor) Process(ctx context.Context, novelID string, job characters.Job) bool {
	if ctx == nil {
	...
	if job.StartedAt == nil || job.Status == "queued" {
		...
		SetCharacterJobProgress(&job, 0, "preparing", nil, nil, nil)
	...
	progressSink := func(progress appcharactersummary.BatchProgress) {
		if ctx.Err() != nil {
			return
		}
		switch progress.Phase {
		case "start":
			SetCharacterJobProgress(&job, CharacterJobBatchProgressPercent(progress.Batch.BatchIndex-1, progress.Batch.BatchCount), "batch", &progress.Batch.BatchIndex, &progress.Batch.BatchCount, nil)
		case "complete":
			completedBatches := progress.Batch.BatchIndex
			if progress.CompletedBatchCount > 0 {
				completedBatches = progress.CompletedBatchCount
			}
			SetCharacterJobProgress(&job, CharacterJobBatchProgressPercent(completedBatches, progress.Batch.BatchCount), "batchComplete", &progress.Batch.BatchIndex, &progress.Batch.BatchCount, &progress.MergedCharacterCount)
		default:
			return
		}
		saveStartedAt := time.Now()
	...
	if err := p.workflow.GenerateAndSave(ctx, novelID, job.RequestedUpToEpisodeIndex, nil, job.GenerationStrategy, progressSink); err != nil {
		...
		SetCharacterJobProgress(&job, ValueOrDefaultInt(job.Progress, 0), "failed", job.CurrentBatchIndex, job.BatchCount, job.GeneratedCharacterCount)
		saveStartedAt := time.Now()
		err := p.store.Save(novelID, job)
		status := "ok"
		if err != nil {
	...
	job.FinishedAt = &finishedAt
	job.ErrorMessage = nil
	SetCharacterJobProgress(&job, 100, "completed", job.CurrentBatchIndex, job.BatchCount, job.GeneratedCharacterCount)
	saveStartedAt := time.Now()
	err := p.store.Save(novelID, job)
	status := "ok"
	if err != nil {
