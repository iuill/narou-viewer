...
func (r *Runtime) Workflow() *appcharactersummary.Workflow {
	return appcharactersummary.NewWorkflow(r)
}

func (r *Runtime) Result(ctx context.Context, novelID string, upToEpisodeIndex string, options appcharactersummary.RequestOptions) (appcharactersummary.Result, error) {
	if r == nil {
		return appcharactersummary.Result{}, nil
	}
	return appcharactersummary.NewService(r.stateDir, r.library, r.settings, r).Result(ctx, novelID, upToEpisodeIndex, options)
}

func (r *Runtime) PreparePreview(ctx context.Context, novelID string, upToEpisodeIndex string, resolvedConfig *store.ResolvedAIGenerationConfig) (appcharactersummary.PreparedPreview, error) {
	if r == nil {
		return appcharactersummary.PreparedPreview{}, nil
	}
	return r.Workflow().PreparePreview(ctx, novelID, upToEpisodeIndex, resolvedConfig)
}

func (r *Runtime) ProcessJob(ctx context.Context, novelID string, job characters.Job) bool {
	if r == nil {
		return false
	}
...
func (r *Runtime) GenerateAndSave(ctx context.Context, novelID string, upToEpisodeIndex string, resolvedOverride *store.ResolvedAIGenerationConfig, strategy string, progressSink func(appcharactersummary.BatchProgress)) error {
	if r == nil {
		return nil
	}
...
func (r *Runtime) GeneratePreview(ctx context.Context, novelID string, upToEpisodeIndex string, resolvedOverride *store.ResolvedAIGenerationConfig, strategy string, progressSink func(appcharactersummary.BatchProgress), episodeIndexes []string, preloaded *appcharactersummary.Inputs) (characters.SummaryResponse, error) {
	if r == nil {
...
func (r *Runtime) Limits() (int, int) {
	return characterSummaryLimits()
}

func (r *Runtime) LoadInputs(ctx context.Context, novelID string, upToEpisodeIndex string, maxChunkChars int, maxBatchChars int, afterEpisodeIndex string) (appcharactersummary.Inputs, error) {
	startedAt := time.Now()
	defer r.log("load_inputs", startedAt, "novelId", novelID, "upToEpisodeIndex", upToEpisodeIndex, "afterEpisodeIndex", afterEpisodeIndex)
	if r == nil || r.library == nil {
		return appcharactersummary.Inputs{}, nil
	}
	toc, err := r.library.GetToc(ctx, novelID)
	if err != nil {
		return appcharactersummary.Inputs{}, err
	}
	if toc == nil {
		return appcharactersummary.Inputs{}, errors.New("character summary generation target novel was not found")
	}
	...
	for _, episodeSummary := range toc.Episodes {
		...
		if episode == nil {
			return appcharactersummary.Inputs{}, errors.New("character summary generation could not load episode bodies")
		}
		...
		text := extractCharacterSummaryEpisodeText(input)
		episodes = append(episodes, characters.HeuristicEpisode{
			EpisodeIndex: episode.EpisodeIndex,
			Text:         text,
			ContentEtag:  episode.ContentEtag,
		})
		chunks = append(chunks, createCharacterSummaryChunksFromText(input, text, maxChunkChars)...)
	}
	return appcharactersummary.Inputs{
		Episodes: episodes,
...
func (r *Runtime) RebatchInputs(ctx context.Context, inputs appcharactersummary.Inputs, config *store.ResolvedAIGenerationConfig, fallbackMaxBatchChars int) appcharactersummary.Inputs {
	startedAt := time.Now()
	result := rebatchCharacterSummaryInputs(ctx, inputs, config, fallbackMaxBatchChars)
	r.log("rebatch_inputs", startedAt, "episodeCount", len(result.Episodes), "batchCount", len(result.Batches), "chunkCount", countCharacterSummaryChunks(result.Batches))
	return result
}
...
func (r *Runtime) GenerateBatch(ctx context.Context, config *store.ResolvedAIGenerationConfig, novelID string, upToEpisodeIndex string, knownCharacters []characters.GeneratedCharacter, batch characterSummaryBatch, unresolvedMentions []characters.GeneratedUnresolvedMention) (appcharactersummary.BatchResult, error) {
	startedAt := time.Now()
	...
	return appcharactersummary.BatchResult{Delta: result.Delta, Usage: result.Usage}, err
}
...
func (r *Runtime) BuildGeneratedPreview(novelID string, upToEpisodeIndex string, generated []characters.GeneratedCharacter, episodes []characters.HeuristicEpisode, episodeIndexes []string, options characters.SaveGeneratedSummaryOptions) (characters.SummaryResponse, error) {
	startedAt := time.Now()
	result, err := buildGeneratedCharacterSummaryPreview(r.stateDir, novelID, upToEpisodeIndex, generated, episodes, episodeIndexes, options)
	status := "ok"
	if err != nil {
		status = "error"
	}
...
func (r *Runtime) BuildHeuristicPreview(novelID string, upToEpisodeIndex string, episodes []characters.HeuristicEpisode, episodeIndexes []string) (characters.SummaryResponse, error) {
	return buildHeuristicCharacterSummaryPreview(novelID, upToEpisodeIndex, episodes, episodeIndexes)
}
...
func (r *Runtime) LoadRequiredPreview(novelID string, upToEpisodeIndex string, episodeIndexes []string) (characters.SummaryResponse, error) {
	return loadRequiredCharacterSummaryPreview(r.stateDir, novelID, upToEpisodeIndex, episodeIndexes)
}
...
func countCharacterSummaryChunks(batches []characterSummaryBatch) int {
	count := 0
...
func rebatchCharacterSummaryInputs(ctx context.Context, inputs appcharactersummary.Inputs, config *store.ResolvedAIGenerationConfig, fallbackMaxBatchChars int) appcharactersummary.Inputs {
	...
	inputs.Batches = createCharacterSummaryBatchesWithBudget(chunks, resolveCharacterSummaryBatchBudget(ctx, config, fallbackMaxBatchChars))
	return inputs
}
