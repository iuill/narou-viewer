...
func newCharacterSummaryOpenRouterTestServer(t *testing.T, responses ...string) *httptest.Server {
	t.Helper()
	index := 0
...
func TestGenerateOpenRouterCharacterSummaryParallelIdentityRejectsMissingConfig(t *testing.T) {
	runtime := NewRuntime(RuntimeDependencies{StateDir: t.TempDir()})
	generated, state, usage, err := runtime.generateOpenRouterCharacterSummaryParallelIdentity(context.Background(), nil, "novel-1", "1", nil, nil, nil, nil)
...
func TestGenerateOpenRouterCharacterSummaryParallelIdentityEmptyInput(t *testing.T) {
	runtime := NewRuntime(RuntimeDependencies{StateDir: t.TempDir()})
	generated, state, usage, err := runtime.generateOpenRouterCharacterSummaryParallelIdentity(context.Background(), &store.ResolvedAIGenerationConfig{}, "novel-empty", "1", nil, nil, nil, nil)
	if err != nil {
		t.Fatalf("generateOpenRouterCharacterSummaryParallelIdentity returned error: %v", err)
	}
...
func TestGenerateOpenRouterCharacterSummaryDiscoveryParallelCorrectionWithSeed(t *testing.T) {
	openrouter := newCharacterSummaryOpenRouterTestServer(t, `{"characters":[{"characterId":"char_seed","canonicalName":"アリス姫","aliases":["姫様"],"keep":true,"reason":"代表名を補正"}]}`)
	...
	generated, state, usage, err := runtime.generateOpenRouterCharacterSummaryDiscoveryParallelCorrection(context.Background(), &store.ResolvedAIGenerationConfig{APIKey: "sk-test", ModelID: "base-model", AllowFallbacks: true}, "novel-d", "1", seed, nil, nil, nil)
	if err != nil {
		t.Fatalf("generateOpenRouterCharacterSummaryDiscoveryParallelCorrection returned error: %v", err)
	}
...
func TestCharacterSummaryWorkflowPortsGenerateParallelIdentityWrapsServer(t *testing.T) {
	runtime := NewRuntime(RuntimeDependencies{StateDir: t.TempDir()})
...
func TestCharacterSummaryWorkflowPortsGenerateDiscoveryParallelCorrectionWrapsServer(t *testing.T) {
	runtime := NewRuntime(RuntimeDependencies{StateDir: t.TempDir()})
...
func TestParallelIdentityRuntimeAndExtractionEmptyInput(t *testing.T) {
	...
	candidates, usage, unresolved, err := runtime.extractParallelIdentityCandidates(context.Background(), config, "novel-1", "1", nil, func(appcharactersummary.BatchProgress) {
...
func TestExtractParallelIdentityCandidatesReturnsFirstBatchError(t *testing.T) {
	...
	batch := characterSummaryBatch{
		BatchIndex: 1,
		Chunks:     []characterSummaryChunk{{EpisodeIndex: "1", Title: "第一話", Text: "アリスが廊下に立っていた。"}},
	}
	candidates, usage, unresolved, err := runtime.extractParallelIdentityCandidates(context.Background(), &store.ResolvedAIGenerationConfig{}, "novel-1", "1", []characterSummaryBatch{batch}, func(item appcharactersummary.BatchProgress) {
...
func TestExtractParallelIdentityCandidatesCollectsSuccessfulBatches(t *testing.T) {
	openrouter := newCharacterSummaryOpenRouterTestServer(
		t,
	...
	runtime := NewRuntime(RuntimeDependencies{StateDir: t.TempDir()})
	progressEvents := []appcharactersummary.BatchProgress{}
	batches := []characterSummaryBatch{
		...
		},
		{
			...
			Chunks:         []characterSummaryChunk{{EpisodeIndex: "2", Title: "第二話", Text: "ボブが庭にいた。"}},
		},
	}
	candidates, usage, unresolved, err := runtime.extractParallelIdentityCandidates(
		...
		"2",
		batches,
		func(progress appcharactersummary.BatchProgress) {
...
func TestParallelIdentityCandidatesFromDeltaWithKnownKeepsUpdatedKnownID(t *testing.T) {
	...
	batch := characterSummaryBatch{BatchIndex: 1, Chunks: []characterSummaryChunk{{EpisodeIndex: "2", Text: "アリスが笑った。"}}}
	candidates := parallelIdentityCandidatesFromDeltaWithKnown("novel-1", 0, batch, characterSummaryDelta{
...
func TestParallelIdentityCandidateHelpers(t *testing.T) {
	fullName := "山田太郎"
	gender := "male"
	batch := characterSummaryBatch{BatchIndex: 2, Chunks: []characterSummaryChunk{{EpisodeIndex: "3", Text: "太郎が来た。"}}}
	candidates := parallelIdentityCandidatesFromDelta("novel-1", 1, batch, characterSummaryDelta{
...
func TestResolveParallelIdentityClustersFallsBackToNameGroups(t *testing.T) {
	openrouter := newCharacterSummaryOpenRouterTestServer(t, `{"clusters":[{"localIds":["b1-c1","b2-c1"],"canonicalName":"アリス","confidence":0.95,"reason":"同じ名前"}]}`)
	defer openrouter.Close()
	t.Setenv("OPENROUTER_API_BASE_URL", openrouter.URL)
...
func TestDiscoverParallelIdentityNamesUsesOpenRouter(t *testing.T) {
	openrouter := newCharacterSummaryOpenRouterTestServer(t, `{"characters":[{"name":"アリス","aliases":["姫"],"episodeIndex":"1","reason":"会話に登場"}]}`)
	defer openrouter.Close()
	t.Setenv("OPENROUTER_API_BASE_URL", openrouter.URL)
	runtime := NewRuntime(RuntimeDependencies{StateDir: t.TempDir()})
	batches := []characterSummaryBatch{{
		...
		BatchCount:     1,
		EpisodeIndexes: []string{"1"},
		Chunks:         []characterSummaryChunk{{EpisodeIndex: "1", Title: "第一話", Text: "アリスが来た。"}},
	}}
...
func TestCorrectParallelIdentityCharactersUsesOpenRouter(t *testing.T) {
	openrouter := newCharacterSummaryOpenRouterTestServer(t, `{"characters":[{"characterId":"char_a","canonicalName":"アリス姫","aliases":["姫様"],"keep":true,"reason":"代表名を補正"}]}`)
	defer openrouter.Close()
	t.Setenv("OPENROUTER_API_BASE_URL", openrouter.URL)
	runtime := NewRuntime(RuntimeDependencies{StateDir: t.TempDir()})
	generated := []characters.GeneratedCharacter{{
...
func TestDiscoveryParallelCorrectionHelpers(t *testing.T) {
	...
	copied := characterSummaryNameDiscoveryConfig(config)
	if copied == config || copied.ModelID != "name-model" || config.ModelID != "base-model" {
		t.Fatalf("copied config = %+v base=%+v", copied, config)
	}
	fallback := characterSummaryNameDiscoveryConfig(&store.ResolvedAIGenerationConfig{ModelID: "base-model", APIKey: "key"})
	if fallback.ModelID != "base-model" {
		t.Fatalf("fallback config = %+v", fallback)
	}
	if characterSummaryNameDiscoveryConfig(nil) != nil {
	...
	payload := characterSummaryBatchPromptPayload(characterSummaryBatch{BatchIndex: 1, BatchCount: 1, EpisodeIndexes: []string{"1"}, Chunks: []characterSummaryChunk{{EpisodeIndex: "1", Title: "第一話", Text: strings.Repeat("あ", 7000)}}})
