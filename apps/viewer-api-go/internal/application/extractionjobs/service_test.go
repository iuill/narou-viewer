...
func TestListAndClearJobs(t *testing.T) {
	...
	if err := extractdomain.SaveJob(stateDir, "novel-1", extractdomain.Job{JobID: "job-done", RequestedUpToEpisodeIndex: "1", Status: "completed", CreatedAt: "2026-01-01T00:00:00Z"}); err != nil {
...
func TestClearRejectsActiveJob(t *testing.T) {
	...
	if err := extractdomain.SaveJob(stateDir, "novel-1", extractdomain.Job{JobID: "job-running", RequestedUpToEpisodeIndex: "1", Status: "running", CreatedAt: "2026-01-01T00:00:00Z"}); err != nil {
		t.Fatalf("SaveJob returned error: %v", err)
	}
	service := NewService(stateDir, fakeLibrary{workFound: true, episodeOK: true}, fakeSettings{})
