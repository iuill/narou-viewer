...
func TestJobCoordinatorProcessJobsConsumesQueuedAndRunningJobs(t *testing.T) {
	...
	processed := []string{}
	coordinator := NewJobCoordinator(stateDir, func(ctx context.Context, novelID string, job characters.Job) bool {
		processed = append(processed, novelID+":"+job.JobID)
		job.Status = "completed"
		if err := characters.SaveJob(stateDir, novelID, job); err != nil {
...
func TestJobCoordinatorKickProcessesWithBackgroundContext(t *testing.T) {
	stateDir := t.TempDir()
	job := characters.Job{JobID: "job-1", RequestedUpToEpisodeIndex: "1", GenerationMode: "heuristic", Status: "queued", CreatedAt: "2026-01-01T00:00:00Z"}
	if err := characters.SaveJob(stateDir, "novel-1", job); err != nil {
		t.Fatalf("SaveJob returned error: %v", err)
	}

	processed := make(chan string, 1)
	coordinator := NewJobCoordinator(stateDir, func(_ context.Context, novelID string, job characters.Job) bool {
...
func TestJobCoordinatorProcessJobsStopsWhenContextCanceled(t *testing.T) {
	...
	cancel()
	called := false
	NewJobCoordinator(t.TempDir(), func(context.Context, string, characters.Job) bool {
		called = true
		return false
	}).processJobs(ctx)
	if called {
