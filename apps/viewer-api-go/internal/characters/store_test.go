...
func TestLoadSummaryAndJobsHandleMissingFiles(t *testing.T) {
	stateDir := t.TempDir()
	if _, ok, err := LoadSummary(stateDir, "missing", "1"); err != nil || ok {
		t.Fatalf("missing summary should return ok=false without error, ok=%v err=%v", ok, err)
	}
...
func TestCharacterHelpersCoverEmptyAndComparisonCases(t *testing.T) {
	if latestHistoryText(nil, "1") != nil {
		t.Fatal("empty history should return nil")
	}
	if textValuePtr(&textVersion{}, "1") != nil {
		t.Fatal("empty text version should return nil")
	}
...
func TestCharacterYAMLWriteErrors(t *testing.T) {
