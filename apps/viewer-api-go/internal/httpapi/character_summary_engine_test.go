...
func TestCharacterSummaryEngineBuildsPromptPreviewChunks(t *testing.T) {
	...
	systemPrompt, userPrompt := buildCharacterSummaryPrompt("novel-1", "2", nil, batches[0], nil)
	...
	if chunks := createCharacterSummaryChunks([]characterSummaryEpisodeInput{{EpisodeIndex: "3", Title: "空話"}}, maxChunkChars); len(chunks) != 1 || chunks[0].Text != "" {
		t.Fatalf("empty episode should still produce an empty chunk: %+v", chunks)
	}
}

func TestCharacterSummaryBatchesUseTokenBudget(t *testing.T) {
	...
	batches := createCharacterSummaryBatchesWithBudget(chunks, characterSummaryBatchBudget{MaxTextTokens: 90})
	...
	if characterSummaryBudgetExceeded(100, 100, characterSummaryBatchBudget{}) {
		t.Fatal("empty character summary budget should not split batches")
	}
}

func TestCharacterSummaryCandidateCardsRankAndLimitCompactCandidates(t *testing.T) {
	...
	batch := characterSummaryBatch{
		Chunks: []characterSummaryChunk{{EpisodeIndex: "3", Title: "第三話", Text: "別名Jが再登場した。"}},
	}
	cards := characterSummaryCandidateCards(values, batch)
	if len(cards) != 8 {
...
func TestCharacterSummaryCandidateCardsKeepAllExactMatches(t *testing.T) {
	values := []characters.GeneratedCharacter{}
	textParts := []string{}
	for i := 0; i < 10; i++ {
	...
	batch := characterSummaryBatch{
		Chunks: []characterSummaryChunk{{EpisodeIndex: "10", Title: "会議", Text: strings.Join(textParts, "、") + "が集まった。"}},
	}
	cards := characterSummaryCandidateCards(values, batch)
	if len(cards) != 10 {
...
func TestCharacterSummaryCandidateCardsDoNotForceSharedShortAliases(t *testing.T) {
	values := []characters.GeneratedCharacter{}
	for i := 0; i < 12; i++ {
		values = append(values, characters.GeneratedCharacter{
	...
	batch := characterSummaryBatch{
		Chunks: []characterSummaryChunk{{EpisodeIndex: "20", Title: "謁見", Text: "王が集まり、青い旅人が名乗った。"}},
	}
	cards := characterSummaryCandidateCards(values, batch)
	if len(cards) != 8 {
...
func TestCharacterSummaryInputsFilterFromEpisode(t *testing.T) {
	...
	filtered := filterCharacterSummaryInputsFrom(inputs, "2")
	...
	if blank := filterCharacterSummaryInputsFrom(inputs, " "); len(blank.Episodes) != 3 {
...
func TestCharacterSummaryPromptIncludesUnresolvedMentions(t *testing.T) {
	batch := characterSummaryBatch{
		EpisodeIndexes: []string{"20"},
		Chunks:         []characterSummaryChunk{{EpisodeIndex: "20", Title: "正体", Text: "アリスが現れた。"}},
	}
	_, userPrompt := buildCharacterSummaryPromptWithUnresolved("novel-1", "20", nil, batch, []characters.GeneratedUnresolvedMention{
	...
	merged := mergeGeneratedUnresolvedMentions([]characters.GeneratedUnresolvedMention{{Mention: "黒衣の男", EpisodeIndex: "10"}}, []characterSummaryUnresolvedMention{
	...
	pending, err := server.loadCharacterSummaryPendingUnresolved("novel-unresolved-cutoff", "12")
	...
	allPending, err := server.loadCharacterSummaryPendingUnresolved("novel-unresolved-cutoff", "")
...
func TestCharacterSummaryMergeProposalRetiresMergedGeneratedID(t *testing.T) {
	allocator := characters.NewGeneratedCharacterIDAllocator("novel-merge-run", nil)
	generated, _ := applyCharacterSummaryDelta("novel-merge-run", nil, characterSummaryDelta{
	...
	generated, _ = applyCharacterSummaryDelta("novel-merge-run", generated, characterSummaryDelta{
		MergeProposals: []characterSummaryMergeProposal{{
			SourceCharacterID: sourceID,
			TargetCharacterID: targetID,
			Confidence:        characterSummaryMergeAutoApplyConfidence,
		}},
	}, allocator)
	state := characterSummaryStateFromAllocator(nil, allocator)
...
func TestGeneratedCharacterSummaryPreviewKeepsExistingEventMentions(t *testing.T) {
	stateDir := t.TempDir()
	...
	summary, err := buildGeneratedCharacterSummaryPreview(stateDir, "novel-preview", "3", []characters.GeneratedCharacter{{
		CharacterID:                 "char_alice",
		CanonicalName:               "アリス",
		CanonicalEpisodeIndex:       "1",
		FirstAppearanceEpisodeIndex: "1",
	...
	if err != nil {
		t.Fatalf("buildGeneratedCharacterSummaryPreview returned error: %v", err)
	}
...
func TestCharacterSummaryCandidateCardsUseFullNameHistory(t *testing.T) {
	oldFullName := "ラビット家の令嬢"
	currentFullName := "アリス・リデル"
	cards := characterSummaryCandidateCards([]characters.GeneratedCharacter{
		{
			CharacterID:                 "char_alice",
			CanonicalName:               "アリス",
	...
	}, characterSummaryBatch{Chunks: []characterSummaryChunk{{EpisodeIndex: "25", Text: "ラビット家の令嬢は古い記録にだけ残っていた。"}}})
	if len(cards) != 1 {
...
func TestReuseGeneratedCharacterIDsFromRegistryKeepsStableIDForReprocessedIdentity(t *testing.T) {
	...
	}, characterSummaryGenerationState{IssuedCharacterIDs: []string{"char_new"}}, "5")
	...
	}, characterSummaryGenerationState{}, "5")
	...
	}, []characters.GeneratedCharacter{{CharacterID: "char_old", CanonicalName: "アリス", FirstAppearanceEpisodeIndex: "1"}}, characterSummaryGenerationState{}, "5")
...
func TestCharacterSummaryHistoryHelpers(t *testing.T) {
...
func TestCharacterSummaryMergeProposalsAreOrderIndependent(t *testing.T) {
	...
	merged, changed := applyCharacterSummaryDelta("novel-1", existing, characterSummaryDelta{
		MergeProposals: []characterSummaryMergeProposal{
	...
	unchanged, changed := applyCharacterSummaryDelta("novel-1", existing, characterSummaryDelta{
		MergeProposals: []characterSummaryMergeProposal{{SourceCharacterID: "char_b", TargetCharacterID: "char_c", Confidence: 0.5, Reason: "低信頼"}},
	}, nil)
...
func TestCharacterSummaryEngineNormalizesRichAndLegacyResponses(t *testing.T) {
	delta := []byte(`{
	...
	normalized, err := normalizeCharacterSummaryOpenRouterResponse(delta, "novel-1", "5")
	if err != nil {
	...
	normalized, err = normalizeCharacterSummaryOpenRouterResponse(rich, "novel-1", "3")
	if err != nil {
	...
	normalized, err = normalizeCharacterSummaryOpenRouterResponse(legacy, "novel-1", "4")
	if err != nil {
	...
	if _, err := normalizeCharacterSummaryOpenRouterResponse([]byte(`{"characters":{}}`), "novel-1", "4"); err == nil {
		t.Fatal("malformed characters payload should fail")
	}
}

func TestCharacterSummaryEngineMergesGeneratedCharacters(t *testing.T) {
	oldFullName := "アリス旧姓"
	fullName := "アリス・スミス"
	oldGender := "不明"
	gender := "女性"
...
func TestCharacterSummaryDeltaAppliesByStableIDOnly(t *testing.T) {
	existing := []characters.GeneratedCharacter{
		{
			CharacterID:                 "char_a",
			CanonicalName:               "佐藤先生",
	...
	updated, changed := applyCharacterSummaryDelta("novel-1", existing, characterSummaryDelta{
	...
	merged, changed := applyCharacterSummaryDelta("novel-1", updated, characterSummaryDelta{
		MergeProposals: []characterSummaryMergeProposal{{SourceCharacterID: "char_b", TargetCharacterID: "char_a", Confidence: 1, Reason: "本文で同一人物と明示"}},
	}, nil)
...
func TestCharacterSummaryDeltaCanonicalUpdateUsesNewerName(t *testing.T) {
	existing := []characters.GeneratedCharacter{{
