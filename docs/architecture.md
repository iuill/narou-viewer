...
8. `viewer-web`: 同一作品を開いている間はチャット履歴と実行ログを保持し、必要に応じてデバッグ用 JSON としてエクスポートできる。境界切替時は会話を新規化する。会話履歴自体にはネタバレ境界を付与しないため、境界の正本はリクエストごとの上限話と `viewer-api` の tool 制限に置く
