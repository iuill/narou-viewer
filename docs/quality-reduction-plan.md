# 過剰品質削減の設計記録: state 保護スタックと sensitive-information 検査基盤

この文書は、個人利用・個人開発 OSS としての narou-viewer に対して過剰になっていた品質対策を削減した際の設計判断と移行計画を記録する。判断基準は [`quality-goals.md`](quality-goals.md) の「品質投資の判断」に従う。
「変更前」はこの削減を実装する PR の base 時点、「現行」は各節を設計した時点の実装を指す。merge 後の運用手順と保証は、[`deployment.md`](deployment.md)、[`state-schema-policy.md`](state-schema-policy.md)、[`quality-goals.md`](quality-goals.md) を正本とする。

- 優先して守るリスク: データ消失、機微情報漏えい、外部課金の再発生、互換性破壊、主要機能の回帰
- 削減してよいもの: 単純な運用手順や既存の外部基盤で代替できる application 固有の仕組み、低減するリスクより維持コストが大きい既存の仕組み

設計時点の `quality-goals.md` は、backup / restore の簡素化を目標として掲げていた。本設計はその実装計画にあたる。
...
代替手順は `deployment.md` へ集約する。
標準 compose の named volume を writer 停止中に one-shot container から checkout 外へ stream し、archive の展開可能性と checksum を確認する。
restore は既存 volume へ上書きせず、新しい空の named volume へ全体を展開し、owner と mode を確認してから起動する。
retention・保存先・暗号化は運用者と外部基盤（暗号化 disk、cloud storage の versioning 等）の責任とする。

専用 tooling の旧 archive は未 release の形式であり、恒久的な互換経路として文書化しない。
...
- 実行時 guard は優先度の高い parse error、unknown version、migration 不整合を read / write 時点で fail-closed に検出し、path・observed / supported version・復旧案を含む error を返す。横断一覧性は失うが、正本への危険な mutation を止める契約は runtime 側に残る。
...
restore journal の起動拒否を外す変更は、一度だけ必要になる upgrade 互換手順を伴う。
専用 tooling は release や tag に含まれず、main へ入ってから撤去 PR までの短期間だけ存在したため、移行手順は実装 PR 本文の「ユーザーへの影響 / upgrade 手順」に限って記載する。
恒久的な運用文書には、専用 tooling に依存しない cold backup、空の named volume への restore、rollback 手順だけを残す。

未 release の旧 archive 形式は継続サポートせず、必要になった場合は git 履歴に残る旧 build と専用 tooling を組み合わせて個別に復旧する。
...
変更前の 2 workflow + App 構成は、fork PR が checkout や status 発行を悪用して required check を偽装・レースさせる攻撃への対策である。具体的には `pull_request_target` から artifact 経由で PR 番号だけを渡し、App token で status を発行し、freshness 照合と head SHA を共有する全 PR の metadata scan まで行う。
...
- backup の暗号化・retention は運用者責任になる。docs に平文 archive を残さない age の stream 例を示して補う。
- restore journal guard 撤去前に現行 build の recovery を実行する必要がある。この一度限りの upgrade 手順は実装 PR 本文の「ユーザーへの影響」に明記し、恒久的な運用文書には残さない。
...
2. **statebackup / restore journal 撤去 phase**
   ...
   - 同じ phase で `deployment.md` の恒久的な手動 backup / restore、実装 PR 本文の一度限りの upgrade 手順、関連 test、`state-schema-policy.md`、`quality-goals.md` の現行差分を更新する。
...
4. **最終整合性 phase**
   - 現行の運用文書と command 索引から、削除済み command / document / Secret Guard 名への参照を除去する。設計判断を説明するこの記録内の変更前名称は残す。
