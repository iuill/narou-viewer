...
- merge 後は `git fetch --prune origin`、base branch への切り替え、`git pull --ff-only origin <base-branch>`、不要になった remote / local branch の削除確認までを一続きの作業として扱います。dirty worktree や別 worktree で使用中の branch は勝手に変更・削除しません。
