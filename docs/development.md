...
- Pull Request の merge には merge commit を使い、squash merge / rebase merge は使いません。エージェントはユーザーから明示的に依頼された場合だけ merge を実行します。
- merge 後は PR の base repository に対応する Git remote で `git fetch --prune`、base branch への切り替え、`git pull --ff-only`、不要になった remote / local branch の削除確認までを一続きの作業として扱います。dirty worktree や別 worktree で使用中の branch は勝手に変更・削除しません。
