...
| `VA-EXTRACTION-JOBS` | `schema_version: 2`、supported legacy なし | 未知 version を一覧では `incompatible` と表示し、自動 queue / resume / recovery / overwrite / prune を拒否。malformed job は対象作品を安全に特定できないため一覧・queue・recovery・全作品の prune を停止 | 対応 build、migration、または明示的な破棄・再投入。状態機械は [#16](https://github.com/iuill/narou-viewer/issues/16) |
