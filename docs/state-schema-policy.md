...
- `ai_usage.sqlite` は migration ledger の future-version guard を read / write / prune に適用し、非対応 DB の bytes と mode を変更しない。`reader_search.sqlite` は version mismatch を検出したら作品単位 mutation ではなく安全な cache rebuild を選べるようにする（[#22](https://github.com/iuill/narou-viewer/issues/22)）。
