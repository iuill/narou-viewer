...
| `VA-EXTRACTION-JOBS` | `schema_version: 2`、supported legacy なし | 未知 version を一覧では `incompatible` と表示し、自動 queue / resume / recovery / overwrite / prune を拒否 | 対応 build、migration、または明示的な破棄・再投入。状態機械は [#16](https://github.com/iuill/narou-viewer/issues/16) |
...
いずれかに unknown version、malformed document、I/O error、partial usage schema があれば最初の mutation 前に全体を拒否する。preflight 後は再生成可能な `reader_search.sqlite` の作品 row を先に削除し、各作品について次の順に処理する。
