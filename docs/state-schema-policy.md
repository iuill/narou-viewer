...
registry で version guard 実装済みとする viewer-api の YAML / JSON state は、schema ごとの guard で version を判定する。共通 guard を使う state は header を typed decode より先に確認し、明示した legacy version 以外の欠落・未知 version・malformed document に対する read / write / prune を拒否する。派生 view / cache だけは registry に従って利用を避け、quarantine / rebuild できる。`VA-READER-AI-PROOFREAD` は JSON decode 後に `formatVersion` を確認し、未知 version を表示に利用しない。
...
| `ai_usage.sqlite` | process-wide write mutex、SQLite transaction、`schema_migrations`、`busy_timeout`、file mode `0600`、`journal_mode=DELETE` | service 停止中の data root copy を前提に rollback journal を明示。複数 process writer は対象外 |
...
- AI credential の encrypted payload と master passphrase は別管理する。master passphrase を data backup に同梱しない。
- data backup は第三者本文、取得済み HTML、画像、読書行動、model output、AI 利用履歴を含み得る。保存時と転送時の暗号化、最小権限のアクセス制御は運用者または外部 backup 基盤が管理する。
...
| `VA-EXTRACTION-JOBS` | `schema_version: 2`、supported legacy なし | current job はfile名と`job_id`の一致、安全な単一componentの`novel_id`、非負整数のrequested boundary、既知statusをtyped decode後に検証し、作品ごとのactive jobを最大1件に制限する。`queued` / `running` / `pausing`をactiveとし、pause後は`paused`、起動時の旧running/pausingは`interrupted`へ確定して明示resumeを待つ。cancelは`canceled`へ確定して自動再実行しない。違反時は一覧・実行・recovery・overwriteを拒否して元bytesを維持する。未知 version を一覧では `incompatible` と表示し、同一作品のcurrent jobも含む自動 queue / background実行 / resume / recovery / overwrite / prune を拒否。未知versionの`novel_id`が文字列かつ安全な単一path componentとして特定できなければbackground実行・recovery・全作品のqueueを停止。malformed job は一覧・queue・recovery・全作品の prune を停止 | 対応 build、migration、または明示的な破棄・再投入。状態遷移と停止境界は [`extraction.md`](extraction.md) を参照 |
...
| `VA-READER-AI-PROOFREAD` | JSON `formatVersion: 1` + `promptVersion: 5` | writer は current version を保存する。reader は JSON decode 後に format version と novel / episode ID を検証し、原文 ETag または prompt version が一致しない結果も表示しない。未知 format version がある場合は生成も拒否して元 file を維持する | 対応 build に戻す。派生 view を破棄してよい場合は、サービス停止後に該当 file を退避または削除してから再生成する |
...
- task state は novel-fetcher owner 内に置き、future-version write fence、request shape、queue invariant、work reservation を起動時と mutation 前に検査する。
...
reader preferences と AI settings は作品非依存なので prune しない。derived character profile / job index / reader search は正本の unknown version fence を妨げず削除できる。再 prune は reading tombstone の世代だけを加算し、他の削除対象を復活させない。複数 file / DB を跨ぐ全体 transaction はないため、preflight 後の I/O failure や process crash では先行変更を rollback しない。orphan / partial failure は次回の runtime access と service log で確認する。
...
runtime repository は parse error と unknown version を読み書き時に fail-closed で検出し、元 file や DB を変更せず、path、observed version、supported version、復旧案を error に含める。
job index、character profile、reader search cache などの派生 state は、各 runtime 経路が正本から再構築または quarantine する。
SQLite の手動 `PRAGMA quick_check` は [`deployment.md`](deployment.md#state-の診断) を参照する。

## 6. backup、restore、rollback

backup は `viewer-api` と `novel-fetcher` を停止し、`state/` と `novel-fetcher/` を含む data root 全体を一度に copy する。
稼働中の file や SQLite を順次 copy する方法と、DB、作品、schema 単位の部分 backup は正式な復旧手段として扱わない。

restore は両 service を停止したまま、空の data root または新しい volume へ backup 全体を復元する。
復元後は backup に対応する build を起動し、その build の startup migration と version guard に検証を委ねる。
現在の data を残した上書き restore と、application binary だけを戻す in-place downgrade はサポートしない。

rollback では upgrade 前に取得した data root 全体と、それに対応する旧 build を組み合わせる。
backup の暗号化、保存先、copy 完了性、retention、復号情報の管理は運用者または外部 backup 基盤が担う。
標準 compose の named volume を使う具体的な backup、restore、診断 command は [`deployment.md`](deployment.md#backup-と-restore) を参照する。
...
- [ ] credentials、第三者本文、model output、AI usage snapshot の file mode、credential 非包含または redaction を確認した
...
| [#24](https://github.com/iuill/narou-viewer/issues/24) | 旧横断診断 tooling として完了後、runtime guard と手動診断へ移行して撤去 |
| [#25](https://github.com/iuill/narou-viewer/issues/25) | 旧専用 tooling として完了後、writer 停止中の data root 全体 copy へ移行して撤去 |
