...
- API usage store は producer から受け取った snapshot をそのまま JSON 化し、credentials 系 key の汎用 redaction は行わない。現行 producer は credential を含めない構造を組み立てる。producer を追加・変更するときは credential 非包含を test するか、明示的な redaction を実装して test する。
...
| `VA-AI-USAGE` | `state/ai_usage.sqlite` | viewer-api / 実装済み | 監査・利用履歴 | 再構築不能。消失しても現在の reader / generation state は壊れないが履歴を失う | 作品紐づき run を削除 | 利用 metadata、会話件数・文字数、制限付き tool I/O、本文 snippet を含み得る producer snapshot |
...
- backup manifest には secret 本体でなく、必要な secret の識別情報または運用メモを記録する。
...
`ai_usage.sqlite` は reader state や AI settings の正本ではないが、producer が投入した token / cost 等の利用情報、request 分類、tool I/O、分析用 snapshot は再生成不能である。現行の読書AI snapshot は会話本文でなく件数・文字数を保存し、tool request / result へ件数・深さ・文字列長の上限を適用する一方、usage store に key 名ベースの汎用 redaction はない。schema mismatch で drop / rebuild せず、[#21](https://github.com/iuill/narou-viewer/issues/21) で transactional migration と snapshot の credential 非包含契約を整備する。
...
1. reading state を tombstone 化
2. bookmarks と novel reader settings を削除
3. character events / profiles、term profiles、extraction jobs / index / checkpoints を削除
4. publications の作品 entry を削除
5. `ai_usage.sqlite` の作品紐づき run / request / snapshot を削除
6. `reader_search.sqlite` の作品紐づき row を削除

reader preferences と AI settings は作品非依存なので prune しない。prune は複数 file / DB に跨るため全体 transaction ではない。

- prune は繰り返し実行しても削除対象が復活せず、安全に収束させる。reading state は stale client の復活を防ぐため再 prune でも tombstone 世代を加算するので、bytes や世代値まで同一になる厳密な idempotency は要求しない。
- orphan は次回 prune または診断で収束できるようにする。
- unknown / unreadable schema を prune が上書き・削除せず、対象と復旧案を報告する。
- reading tombstone を安易に物理削除しない。
- state doctor では orphan、job / index mismatch、frontier inversion、unknown version を検査する。実装は [#24](https://github.com/iuill/narou-viewer/issues/24) で追跡する。
...
full backup では `state/` と `novel-fetcher/` を同じ snapshot generation として取得する。「同じ時刻に copy を開始した」だけでは一貫性を保証しない。
...
backup manifest には timestamp、snapshot generation ID、application build、schema ID と observed version、含めた / 省略した group、hash または snapshot ID、必要な secret の識別情報、snapshot method を記録する。
...
| [#21](https://github.com/iuill/narou-viewer/issues/21) | `ai_usage.sqlite` の番号付き migration と future-version guard |
...
| [#24](https://github.com/iuill/narou-viewer/issues/24) | state doctor / reconciliation command |
| [#25](https://github.com/iuill/narou-viewer/issues/25) | consistent backup / restore tooling と manifest |
