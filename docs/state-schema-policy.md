...
自動 repair は `--apply` と dry-run report の finding ID 個別指定を必須とし、job index、character profile、reader search cache の quarantine / rebuild だけを許可する。`--apply` は走査から repair 後の再走査まで viewer-api writer lock を保持し、稼働中 writer があれば mutation 前に拒否する。正本・生成正本・監査履歴・unknown future version は diagnostic-only である。
