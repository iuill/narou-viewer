...
| `VA-EXTRACTION-JOBS` | `schema_version: 2`、supported legacy なし | 未知 version を一覧では `incompatible` と表示し、同一作品のcurrent jobも含む自動 queue / background実行 / resume / recovery / overwrite / prune を拒否。未知versionで作品IDを特定できなければbackground実行・recovery・全作品のqueueを停止。malformed job は一覧・queue・recovery・全作品の prune を停止 | 対応 build、migration、または明示的な破棄・再投入。状態機械は [#16](https://github.com/iuill/narou-viewer/issues/16) |
