...
公開 API は複数の対象を配列で受け取りますが、受理時に 1 resource ずつ別 task へ分割します。
永続 request は `download` なら 1 target、`update` / `resume` なら 1 work ID だけを保持します。
task summary の `targets` / `novelIds` は Service API 契約との互換性のため配列で返しますが、1 task に複数 resource は含めません。
...
task 状態の正本は SQLite だけです。
`taskqueue.Queue` は永続 repository と wake 通知をまとめる adapter であり、別の queue 状態を memory に保持しません。

control と claim handoff は task ID 単位で直列化します。
runner は `ClaimNext` 後の `requested_action` を読んでから executor を開始するため、開始境界でも SQLite が受理した action を context cancellation に反映します。
...
executor の終了後から SQLite の terminal 更新が完了するまでは、runner が旧 attempt を保持します。
この間の新しい pause、cancel、resume は `409 Conflict` であり、`changed:true` として受理しません。
既に受理済みの同じ pause または cancel を再送した場合だけ、SQLite の同じ action を `changed:false` として返します。
...
1 task は 1 作品だけを処理し、その作品の完了と同時に `execution_committed` を立てます。
永続 request の kind と resource 形状が一致しない場合は、startup recovery と実行開始時の双方で拒否します。
