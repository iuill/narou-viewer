...
この文書は、キャラクター一覧生成の prompt / model 比較実験を保存し、後からレビューするための開発用評価・チューニング手順である。現行の CLI と review skill の入口をまとめる。
...
- [`scripts/run-character-summary-experiment.mjs`](../scripts/run-character-summary-experiment.mjs)
- [`.agents/skills/character-summary-run-review/SKILL.md`](../.agents/skills/character-summary-run-review/SKILL.md)
...
入口は root script の `experiment:character-summary:run`。
...
- `--up-to-episode-index`: キャラクター一覧生成の上限話
...
保存済み run の比較には `character-summary-run-review` skill を使う。
