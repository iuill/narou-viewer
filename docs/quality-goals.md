...
- 最新 release は、直前の対応 release が生成した data から直接 upgrade できることを保証する。
- 複数 release を飛ばす upgrade は原則としてサポートしない。
...
- 未知の将来 schema は fail-closed で拒否し、元の file や DB を変更しない。
...
- 不完全な restore 先や、現在の data を残したまま上書きした restore 先から application を起動しない。
...
専用 archive、暗号化、manifest、自動 restore、restore journal は、最低限の品質目標には含めない。
追加 tooling として維持する場合も、writer 停止中の data root 全体 copy を基準となる復旧経路から外さない。
...
- character events、term profiles、stable character ID、merge 履歴は生成正本として扱う。
- 再構築可能な character profile、job index、reader search index は派生データまたは cache として扱う。
- job と checkpoint は、重複 request、重複適用、課金再発生を防ぐ運用 state とする。
...
- `viewer-web`、`viewer-api`、`novel-fetcher` 単位の coverage gate は維持する。
- coverage gate の達成だけで品質を満たしたとは判断しない。

重点テストは次の境界へ配置する。

- **parser**：外部 HTML、embedded JSON、URL の変更や malformed input を誤って保存しない。
- **schema**：直前 release の migration、未知 version の拒否、拒否時の zero mutation を確認する。
...
- **backup / restore**：停止中の全体 copy、不完全な restore 先の拒否、旧 data を使う rollback を確認する。
...
| 項目 | 現行 | 目標 |
| --- | --- | --- |
| backup / restore | 専用の暗号化 archive、full-generation restore、journal recovery を正式経路として提供 | writer 停止中の data root 全体 copy を基準経路とし、専用 tooling は必須にしない |
...
この表の目標は、対応する実装と機能仕様が更新されるまで現行挙動を置き換えない。
