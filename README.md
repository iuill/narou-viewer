...
```bash
docker compose -p narou-viewer -f docker-compose.prod.yml up --build --remove-orphans
```
...
旧 4 containers 構成から更新する場合も同じ project 名で起動してください。`--remove-orphans` で旧 `reverse-proxy` を削除し、`viewer-web` に同じ公開 port を引き継ぎます。named volume の runtime data は維持します。
