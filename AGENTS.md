...
- Betterleaksの`gitleaks:allow` / `betterleaks:allow` markerは信頼せず、すべての検査経路で無効化する。PRのcommit rangeは権限を`contents: read`に限定した`Sensitive Information / Scan commits`をrequired checkとし、PR metadataは自動検査しない。maintainerはtitle、body、commentとscanner / workflow変更をmerge前に確認し、自動mergeやreview bypassを使わない。
