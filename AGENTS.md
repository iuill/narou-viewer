...
- PR の merge はユーザーが明示的に依頼した場合だけ行い、merge commit を使用する。merge 後は `.agents/skills/pr-merge/SKILL.md` に従い、base branch の local HEAD を remote HEAD へ同期し、安全に削除できる remote / local の作業 branch を片付けてから完了とする。
